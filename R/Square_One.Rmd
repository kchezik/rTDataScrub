...
prob_plot = function(mod, date, temperature, smooth = T, err_perc = NULL){
  ...
  #Extract ordingal "day" of dataset
...
inits = create_inits(air_A = df$coefs$air_A, air_alpha = df$coefs$air_meanT, n_sites = length(unique(df$daily$site)), chains = 1)
...
mod = sampling(compile, data= list(N = length(df$daily$temperature),
                                 ...
                                 prior = 1),
               init = inits, iter = 30, chains = 1)

shinystan::launch_shinystan(mod)

#Coefficients
global = broom::tidy(mod, pars = c("alpha_max","m","k","w_low"), "median") %>%
  select(-std.error) %>% 
  spread(data = ., key = term, value = estimate)

broom::tidy(mod, pars = c("A"), "median") %>%
  select(-std.error) %>% pull(estimate)

cf = broom::tidy(mod, pars = c("alpha","A","tau_est","sigma","sigma_g"), "median") %>%
  select(-std.error) %>% 
  mutate(
...
for(i in 1:nrow(cf)){
  if(cf[i,"coef"]!="tau_est" & !is.na(cf[i,"model"])){
    if(cf[i,"model"]==1) cf[i,"model"] = 2
    else cf[i,"model"] = 1
  }
  if(cf[i,"coef"] == "alpha_air") {
    cf[i,"coef"] = "alpha"
    cf[i,"model"] = 1
  }
  if(cf[i,"coef"] == "alpha_water") {
    cf[i,"coef"] = "alpha"
    cf[i,"model"] = 2
  }
  if(cf[i,"coef"] == "sigma_g") {
    cf[i,"model"] = 2
  }
}

cf = spread(data = cf, key = coef, value = estimate)

plot(df$coefs$water_meanT,cf[cf$model == 2,"alpha"], xlim = c(0,10), ylim = c(0,15))
abline(1,1)

#Mean water and ground temperature fit.
temp = cf %>% filter(model == 2) %>% left_join(df$daily, ., by = "site") %>% 
  mutate(mean_w = alpha + A*cos(2*pi*d/365+tau_est*pi),
         mean_g = alpha)
...
temp = temp %>% mutate(
  ...
  lower_g = mean_g + 1.96*sigma_g) %>% select(-c(6:11))

#Mean air temperature fit.
temp = cf %>% filter(model == 1) %>% left_join(temp, ., by = "site") %>% 
...
#Plot data, mean estimate and error.
ggplot() +
  geom_ribbon(data = temp, aes(x = time, ymin = lower_g, ymax = upper_g), fill = "green", alpha = 0.3) +
  geom_ribbon(data = temp, aes(x = time, ymin = lower_a, ymax = upper_a), fill = "red", alpha = 0.3) +
  geom_ribbon(data = temp, aes(x = time, ymin = lower_w, ymax = upper_w), fill = "blue", alpha = 0.3) +
  geom_point(data = filter(temp, source == "air"), aes(time, temperature), color = "red", size = .5) +
  geom_point(data = filter(temp, source == "ground"), aes(time, temperature), color = "green", size = .5) +
  geom_point(data = filter(temp, source == "water"), aes(time, temperature), color = "blue", size = .5) +
  geom_line(data = temp, aes(time, mean_g), color = "green", size = .5, alpha = .5) +
  geom_line(data = temp, aes(time, mean_a), color = "red", size = .5, alpha = .5) +
  geom_line(data = temp, aes(time, mean_w), color = "blue", size = .5, alpha = .5) +
  geom_hline(yintercept = 0) +
  facet_wrap(~site) + ggthemes::theme_tufte()
```
