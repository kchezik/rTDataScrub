...
These functions create simulated data and offer tools to estimate parameters quickly as priors to subsequent models.

```{r Generative Functions}
# Create annual temperature curve.
alpha_est = function(){
...
create = function(A, AR, s_t=c(0,0), meanT, years = 1, tao){
  N = years*365 * 24 # Number of samples.
  d_cycle = cos((years*2)*pi*1:N/N + tao*pi) # Generic season cycle.
  sd = exp(d_cycle)*s_t[1] # Changing standard deviation by season.
  h_cycle = sd*exp(cos((years*2)*pi*1:N/24 + tao*pi)) # Daily cycle by hour.
  noise = arima.sim(model = list(ar = AR), sd = sd*s_t[2], n = N) # Seasonally adjusted AR1 correlated noise.
  (d_cycle * A + meanT) + h_cycle + noise #Return combined data.
}
# Simulate water infiltrated ground temperature data.
ground = function(years = 1, t0 = 4.5, theta = 0.5, sigma = .3){
  N = years*365*24
  t0 = log(t0)
  epsilon <- vector(length = N, mode = "numeric")
  y <- vector(length = N, mode = "numeric")
  
  y[1] <- rnorm(1, t0, 1)
  epsilon[1] <- y[1] - t0
  for (i in 2:N) {
    y[i] <- exp(rnorm(1, t0 + theta * epsilon[i - 1] - (sigma^2)/2, sigma))
    epsilon[i] <- log(y[i]) - t0 - theta * epsilon[i - 1] + (sigma^2)/2
  }
  y
}
...
generate = function(air_ends = F, air = F, ground_sat = F, years = 1, randomize = c(T,T,T), global){
  library(tidyverse); library(lubridate)
  ...
  air_s_t = c(rlnorm(1,log(1),.07), rlnorm(1,log(.5),.07)) # d_sd N(1,.2) and h_sd N(.5,.2)
  ...
  alpha_inf = 13.5
  if(water_meanT<alpha_inf) water_A = water_meanT
  else water_A = alpha_inf - (water_meanT-alpha_inf)
  
  water_AR = rbeta(n = 1, shape1 = 400, shape2 = 20)
  water_s_t = c(rlnorm(1,log(0.3),.07), rlnorm(1,log(.5),.07)) # d_sd N(.3,.05) and h_sd N(.5,.1)
  #Ground as some function of the water mean temperature.
  ground_meanT = rlnorm(1,log(water_meanT),.2)
  #Seasonal Period Parameter
  tao = rbeta(1,1,1) # Randomize sampling period.
  #Create Data.
  df = data.frame(
    air = create(A = air_A, AR = air_AR, s_t = air_s_t, meanT = air_meanT, years = years, tao = tao),
    water = create(A = water_A, AR = water_AR, s_t = water_s_t, meanT = water_meanT, years = years, tao = tao),
    ground = ground(years = years, t0 = ground_meanT), # Generate ground data.
  ...
  #Add saturated earth data.
  if(randomize[3] == T & ground_sat == F) {
    ans = sample(x = c(0,1), size = 1, prob = c(.7,.3))
    if(ans == 1) ground_sat = T
    ans = 0
  }
  if(ground_sat == T){
    cc = 2
    while(cc != 1){
      ground = period(df,3)
      cc = length(unique(diff(ground$time)))
    }
    ground = mutate(ground, source = "ground")
    df = reduce(df, ground)
  }
  ...
  if(ground_sat == T) fin = suppressWarnings(bind_rows(ground, fin))
  if(air == T) fin = suppressWarnings(bind_rows(fin, airT))
  fin = arrange(fin, time)
  
  cfs = data.frame(latitude, tao,
                   alpha_max = global$alpha_max, 
                   k = global$k, b = global$b, 
                   aw_infl = global$m,
                   S_Adj_SD = water_s_t[1], ER_Adj_SD = water_s_t[2],
                   air_AR,water_AR,
                   air_A, water_A,
                   air_meanT, water_meanT,
                   ground_meanT, theta = 0.5)
  list(dat = fin, coefs = cfs)
}

multi_site = function(n = 1, years = 1, air_ends = F, air = F, ground_sat = F, randomize = c(T,T,T)){
  ...
  for(i in 1:n){
    ...
    df = generate(air = air, ground_sat = ground_sat, air_ends = air_ends, 
...
create_inits = function(K = 3, A_ij = c(.9,.1,.1,.1,.9,.1,.1,.1,.9), p_1k = c(.495,.495,.01),
                        alpha_max = 27, w_low = 0, k = .15, m = 16,
                        water_A = 4, air_A, air_alpha, chains = 1, n_sites){
  #browser()
  #Create single chain list
  init = list(p_1k = matrix(data = p_1k, nrow = n_sites, ncol = K, byrow = T),
              A_iJ = matrix(data = A_ij, nrow = K, ncol = K),
              #alpha_max = alpha_max-w_low, k = k, w_low = w_low, m = m,
              A = matrix(data = c(rep(water_A, n_sites),air_A), nrow = n_sites, ncol = 2),
              #alpha = matrix(data = c(rep(4, n_sites), air_alpha), nrow = n_sites, ncol = 2)
              alpha_air = air_alpha,
              alpha_water = if_else(air_alpha>0,air_alpha,2))
  init$A[,1] = if_else(init$alpha_water<13.5, init$alpha_water, (27-init$alpha_water))
...
prob_plot = function(mod, date, temperature, smooth = T, err_perc = NULL){
  #Build dataframe
  ...
  #Coefficients
  cf = broom::tidy(mod) %>% select(-std.error) %>% spread(term,estimate)
  ...
  #Add Model Fits
  df2$water_fit = cf$alpha_water + cf$`A[1]`*cos(2*pi*df2$day/nrow(df2) + cf$`tau_est[2]`*pi)
  df2$water_sd = (cos(2*pi*df2$day/nrow(df2) + cf$`tau_est[2]`*pi)+2) * cf$`sigma[1]`
  if(!is.null(err_perc)){
  ...
  #Probability plot
  p1p = ggplot(df1) + 
    geom_line(aes(date, median), size = .3, linetype = 2) +
    geom_ribbon(aes(date, ymin = lower, ymax = upper), alpha =.3, fill = "royalblue") +
    ggthemes::theme_tufte() +
    labs(x = "", y = "Probability")
  
  #Raw/Probability
  x_pos = max(which(month(df1$date)==2)/nrow(df1))
  p1 = ggplot(df1) + 
    geom_line(aes(date, temperature)) +
    geom_point(aes(date, temperature, col = median)) +
    viridis::scale_color_viridis() +
    ggthemes::theme_tufte() +
    theme(legend.position = c(x_pos, .85), legend.direction = "horizontal") +
    labs(color ="Probability Water", x = "", y = "Temperature") +
    guides(colour = guide_colourbar(title.position="top"))
  
  #Best Fit
  p1f = ggplot(df2, aes(date,temperature))
  
  if(is.null(err_perc)){
    p1f = p1f + 
      geom_point(aes(date, temperature, color = est_source), shape = 19) +
      labs(color = "", x = "Date", y = "Temperature")
  } 
  if(!is.null(err_perc)){
    p1f = p1f +
      geom_point(data = filter(df2, err == 19), aes(date, temperature, color = est_source), shape = 19) +
      geom_point(data = filter(df2, err == 1), aes(date, temperature, color = est_source), shape = 1) +
      labs(color = paste(as.character(percent_error),"% error",sep=""), x = "Date", y = "Temperature")
  }
  
  p1f = p1f + geom_line(aes(date, water_fit)) +
    geom_ribbon(aes(date, ymin = water_fit - 1.97*water_sd, ymax = water_fit + 1.97*water_sd), 
                alpha = .3, fill = "royalblue") +
    ggthemes::theme_tufte() +
    theme(legend.position = c(x_pos, .85), legend.direction = "horizontal") + 
    guides(color = guide_legend(title.position = "top",title.hjust = .5))
  
  gridExtra::grid.arrange(p1p, p1, p1f, layout_matrix = matrix(data = c(1,2,3), nrow = 3, ncol = 1))
}
...
df$daily = df$daily %>% mutate(y_hat = cf$alpha + cf$A*cos(2*pi*d/365 + cf$tau*pi),
                    sd = 1.96*(cos(2*pi*d/365 + cf$tau*pi)+2)*cf$sigma)
...
air = air %>% mutate(d = c(1:length(temperature)),
                    ...
                    sd = 1.96*(cos(2*pi*d/length(temperature) + cf$tau*pi)+2)*cf$sigma)
...
The models fit the real water and air temperature well and capture the major differences between the two temperature sources. This is encouraging that the models, though the same, may have such different parameter values they will be seperable in the HMM.

Now I'll build a saturated earth (i.e., buried logger) model, simulate the data and recapture those values.

```{r Recapture Simulated Ground Temperature}
#create data
g_dat = ground()

#Compile and run model.
compile = stan_model("./stan/Const_sim.stan")
mod = sampling(compile, data= list(N = length(g_dat), y = g_dat),
                warmup = 300, iter = 800, chains = 4)

#Coefficients
cf = broom::tidy(mod, pars = c("t0","theta","sigma"), "mean") %>%
  select(-std.error) %>% spread(term,estimate)

#collect results
y_hat = vector(mode = "numeric", length = length(g_dat))
sapply(X = c(1:length(g_dat)), FUN = function(x){
  if(x == 1){
    y_hat[x] <<- cf$t0 + rnorm(1,0,cf$sigma)
  } 
  else {
    y_hat[x] <<- cf$t0 + cf$theta*(y_hat[x-1]-cf$t0) + rnorm(1,0,cf$sigma)
  }
})
g_data = data.frame(ground = g_dat, ground_hat = y_hat, time = c(1:length(g_dat)))

#plot results
ggplot() + 
  geom_point(data = g_data, aes(time, g_dat))+
  geom_point(data = g_data, aes(time, y_hat), color = "purple", alpha = 0.2)
```

Although real water-saturated ground-temperature-influenced data would be highly correlated and described by lognormal errors, I've decided to simply capture it's essence and use just a y-intercept model with normal errors and no AR or MA component. By avoiding lognormal errors, we'll be able to incorporate this model into our HMM without worry of INF values when temperature data go negative. By not using an MA or AR component we loose some accuracy and realism but are more likely to have improved fit times as the values that describe these terms can vary wildly and capture a lot of variation that may be attributed to other data sources.

Now we need to combine the air, water and ground temperature data and build a Hidden Markov Model that pulls them apart. In considering an eventual multi-site heirarchical model, we need a global model that describes how air and water temperature are loosely related to help the model find a fit and limit the data to physically reasonable boundaries. Here I propose a model where water temperature follows air temperature, often laging below air temperature but asymptoting at 0 and ~27ºC. This model is the same as a logistic curve where the parameters are related to temperature.

```{r Global Model}
#Gather/generate multi-site data.
df = multi_site(n = 100, years = 1, air_ends = F, air = F, ground_sat = F, randomize = c(F,F,F))

#Compile and run model.
compile = stan_model("./stan/global.stan")
mod = sampling(compile, data= list(S = length(unique(df$daily$site)),
...
#Functions to scale values between 0 and 1 for logit transformation
range01 <- function(x){(x-(min(x)-0.001))/(max(x)-min(x)+0.002)}
inv.range01 = function(x,y){(x*(max(y)-min(y)+0.002) + (min(y)-0.001))}

#Compare mean and median estimates with mean(logit) transformed estimates.
data.frame(rstan::extract(mod, pars = c("alpha_water"))) %>%
  gather(data = ., key = "term", value = "alpha_water") %>% 
  group_by(term) %>% 
  summarize(mean = mean(alpha_water),
            median = median(alpha_water),
            alpha = inv.range01(boot::inv.logit(mean(psych::logit(range01(alpha_water)))),alpha_water))

#Diagnostic Global Model
#Estimate
broom::tidy(mod,pars = c("alpha_max", "m", "k","w_low")) %>% select(-std.error) %>% 
  spread(data = ., key = term, value = estimate); cf
#Actual
df$coefs %>% select(alpha_max,k,aw_infl,b) %>% colMeans(.)

cf = broom::tidy(mod, "alpha_water") %>% select(-std.error) %>% 
  mutate(site = as.numeric(stringr::str_extract_all(string = term, patter = "\\d+", simplify = T))) %>%
  select(-term)

#Plot Global Model (red = air, blue = water, black = actual)
df$coefs %>% select(latitude, air_meanT, water_meanT, site) %>% left_join(cf,.) %>% 
  ggplot(data = ., aes(latitude, estimate)) + geom_smooth(se = F) + geom_point(color = "blue", alpha = .5) + 
  geom_point(aes(latitude, water_meanT), color = "black") +
  geom_smooth(aes(latitude, air_meanT), color = "red", se = F)
```

This gobal model fits very quickly and seems very reasonable, however it is unclear whether this will fit well to real data. It is important to remember that air estimates will be coming from models that already account for latitude, elevation and distance to large water water bodies. Therefore, we can assume these variables are accounted for when estimating water temperature from air temperature. I'm struggling with allowing for more variation/uncertainty in the model. I would prefer site specific data to drive the relationship when available but also have the influence of the global model to keep estimates reasonable especially when data is limited.

Now we can build the HMM for a single site with three temperature sources where water and air temperature data have a described relationship. Thinking about the physical relationships between data sources, I've concluded that mean water and ground temperature are likely the same. I've thus allowed these variables to be shared between the two models. This should spead up fit times. I've also realized that sigma for water and air do not capture similar information as sigma in the ground model, so I've broken them apart and no longer order ground beneath water and air.

```{r Recapture Simulated Multi-Sources w/HMM}
df = multi_site(n = 1, years = 1, air_ends = T, air = T, ground_sat = T, randomize = c(F,F,F))
inits = create_inits(water_A = 4, air_A = df$coefs$air_A, air_alpha = df$coefs$air_meanT, chains = 4, n_sites = length(unique(df$daily$site)))

#Compile and run model.
compile = stan_model("./stan/HMM.stan")
mod = sampling(compile, data= list(N = length(df$daily$temperature), K = 3, 
                                           y = df$daily$temperature,
                                           tau = tau_init(time = df$daily$time), 
                                           n = n_init(time = df$daily$time),
                                           air_A = df$coef$air_A,
                                           air_mean = df$coef$air_meanT),
...
I'm again realizing that auto-regressive and moving average terms are not condusive to HMMs in this context. The parameter controling the influence of the previous data point(s) is too general making it really easy for chains to find local optimums. In other words, these parameters are to flexible and don't constrain the models enough, resulting in a lot of overlap.
...
test = read_rds(path = "./Data/test.rds") %>% 
  mutate(date = lubridate::round_date(DateTime, "day")) %>% 
  arrange(date) %>% group_by(date) %>% 
  summarise(temperature = mean(Temperature)) %>% 
  mutate(day = c(1:length(date)))

inits = list(list(p_1k=c(.495,.495,.01), A = c(15,25), alpha_air = 10, alpha_water = 5),
             list(p_1k=c(.495,.495,.01), A = c(15,25), alpha_air = 10, alpha_water = 5),
             list(p_1k=c(.495,.495,.01), A = c(15,25), alpha_air = 10, alpha_water = 5),
             list(p_1k=c(.495,.495,.01), A = c(15,25), alpha_air = 10, alpha_water = 5))
#Compile Model
compile = stan_model("./stan/HMM.stan")
#Run model.
mod = sampling(compile, data= list(N = nrow(test), K = 3, 
                                           y = test$temperature,
                                           tau = tau_init(time = test$date), 
                                           n = n_init(time = test$date),
                                           air_A = 22.9,
                                           air_mean = 5.2),
               init = inits, warmup = 500, iter = 1000, chains = 4)

#Plot
prob_plot(mod, test$date, test$temperature, smooth = F)
```

In the real data, if we don't provide air mean and amplitude estimates the model appears to split the water temperature data in two. Thereby sharing the same results with only marginal differences. I suspect this could be problem when a site has no air temperature data to fit. By estimating the amplitutde and mean annual air temperature at a site via the BC Climate module and lat./long. information, we can guarentee one model to be air temperature and the other water. After fitting the model where the vertical adjustement and amplitude of the air temperature model are clearly defined we get the temperature models fitting the data correctly and even capture the single air temperature values at the beginning and end of the dataset. 

It's notable that the suspected ground data pulls the water temperature model down artificially and somewhat miss-labels the tail ground temperature data and some of the data transitioning to ground from water. This problem may be resolved by sharing data across sites in a hierarchical framework or in combination.
...
```{r Smoothed HMM}
#Compile Model
compile = stan_model("./stan/HMM_smooth.stan")
#Run model.
mod = sampling(compile, data= list(N = nrow(test), K = 3, 
                                           y = test$temperature,
                                           tau = tau_init(time = test$date), 
...
#Plot
prob_plot(mod, test$date, test$temperature, smooth = T)
```
...
df = multi_site(n = 20, years = 1, air_ends = F, air = F, ground_sat = F, randomize = c(F,F,F))
#Compile and run model.
compile = stan_model("./stan/water_rand.stan")
...
#Coefficients
global = broom::tidy(mod, pars = c("alpha_max","k","m","w_low"), "median") %>%
  select(-std.error) %>% 
  spread(data = ., key = term, value = estimate)

cf = broom::tidy(mod, pars = c("alpha_water","A","tau_est","sigma"), "median") %>%
  select(-std.error) %>% 
...
temp = df$daily %>% left_join(.,cf, by = "site") %>% mutate(
  mean = alpha_water+A*cos(2*pi*d/365+tau_est*pi))

#Variance around mean water temperature.
temp = temp %>% mutate(
  upper = mean + 1.96*(cos(2*pi*d/365 + tau_est*pi)+2)*sigma,
  lower = mean - 1.96*(cos(2*pi*d/365 + tau_est*pi)+2)*sigma)
...
df = multi_site(n = 20, years = 1, air_ends = T, air = T, ground_sat = F, randomize = c(F,F,F))

#Gather initial values.
inits = create_inits(air_A = df$coefs$air_A, air_alpha = df$coefs$air_meanT, n_sites = length(unique(df$daily$site)), chains = 4)

#Compile and run model.
compile = stan_model("./stan/HMM_rand_AH.stan")
mod = sampling(compile, data= list(N = length(df$daily$temperature),
                                 S = length(unique(df$daily$site)),
                                 site = df$daily$site,
                                 K = 3,
                                 ...
                                 prior = 1),
               init = inits, iter = 100, chains = 4)

shinystan::launch_shinystan(mod)

#Coefficients
cf1 = broom::tidy(mod, pars = c("A","tau_est","sigma"), "mean") %>%
  select(-std.error) %>% 
  mutate(
...
cf2 = broom::tidy(mod, pars = c("alpha_w","alpha_a","t0","sigma_g"), "mean") %>%
  select(-std.error) %>% 
  mutate(site = as.numeric(stringr::str_extract_all(string = term, pattern = "\\d+(?=\\])",simplify = T)),
         model = c(rep(1,20),rep(2,20),rep(3,40)),
         coef = stringr::str_extract_all(term, "\\w+", simplify = T)[,1]) %>% 
  select(-term)

cf2[cf2$coef=="alpha_w" | cf2$coef=="alpha_a","coef"] = "alpha"

cf = bind_rows(cf1,cf2) %>% spread(key = coef, value = estimate)

#Mean water temperature real vs. estimate
plot(df$coefs$water_meanT, cf[cf$model == 1,"alpha"], xlim = c(0,10), ylim = c(0,10))
text(df$coefs$water_meanT, cf[cf$model == 1,"alpha"], labels = cf[cf$model == 1,"site"], pos = 1)
abline(0,1)
points(df$coefs$water_meanT, inits[[1]]$alpha_water, col = "red")
#Water temperature amplitude real vs. estimate
plot(df$coefs$water_A, cf[cf$model == 1,"A"], xlim = c(0,10), ylim = c(0,10))
text(df$coefs$water_A, cf[cf$model == 1,"A"], labels = cf[cf$model == 1,"site"], pos = 1)
abline(0,1)
points(df$coefs$water_A, inits[[1]]$A[,1], col = "red")

#Mean water temperature fit.
temp = cf %>% filter(model == 1) %>% left_join(df$daily, ., by = "site") %>% 
  mutate(mean_w = alpha + A*cos(2*pi*d/365+tau_est*pi)) %>% select(-c(6:8,10:11))
#Mean ground temperature fit.
temp = cf %>% filter(model == 3) %>% select(site,sigma_g,t0) %>% left_join(temp, ., by = "site") %>% 
  mutate(mean_g = t0) %>% select(-10)

#Variance around mean water and ground temperature.
temp = temp %>% mutate(
  upper_w = mean_w + 1.96*(cos(2*pi*d/365 + tau_est*pi)+2)*sigma,
  lower_w = mean_w - 1.96*(cos(2*pi*d/365 + tau_est*pi)+2)*sigma,
  upper_g = mean_g + 1.96*sigma_g,
  lower_g = mean_g + 1.96*sigma_g) %>% select(-c(6,7,9))

#Mean air temperature fit.
temp = cf %>% filter(model == 2) %>% left_join(temp, ., by = "site") %>% 
  mutate(mean_a = alpha + A*cos(2*pi*d/365+tau_est*pi))

#Variance around mean air temperature.
temp = temp %>% mutate(
  upper_a = mean_a + 1.96*(cos(2*pi*d/365 + tau_est*pi)+2)*sigma,
  lower_a = mean_a - 1.96*(cos(2*pi*d/365 + tau_est*pi)+2)*sigma) %>% 
  select(-c(12:18))
...
xi_est %>% filter(day %in% which(df$daily$site==18)) %>% select(est_source, day, median) %>% 
  spread(., key = est_source, value = median) %>% arrange(day) %>% print.data.frame()
...
df2 = df2 %>% mutate(err = if_else(source == est_source, 1,19))
percent_error = df2 %>% group_by(site) %>% 
  summarise(percent_error = round((1-sum(source==est_source)/nrow(df2))*100))

#Plot data, mean estimate and error.
ggplot(data = df1) +
  #geom_ribbon(aes(x = time, ymin = lower_g, ymax = upper_g), fill = "green", alpha = 0.3) +
  geom_ribbon(aes(x = time, ymin = lower_a, ymax = upper_a), fill = "red", alpha = 0.3) +
  geom_ribbon(aes(x = time, ymin = lower_w, ymax = upper_w), fill = "blue", alpha = 0.3) +
  geom_point(aes(time, temperature, color = median), size = .5) +
  #geom_line(aes(time, mean_g), color = "green", size = .5, alpha = .5) +
  geom_line(aes(time, mean_a), color = "red", size = .5, alpha = .5) +
  geom_line(aes(time, mean_w), color = "blue", size = .5, alpha = .5) +
  geom_hline(yintercept = 0) +
  viridis::scale_color_viridis() +
  ggthemes::theme_tufte() +
  theme(legend.direction = "horizontal") +
  labs(color ="Probability Water", x = "Date", y = "Temperature") +
  guides(colour = guide_colourbar(title.position="top")) +
  facet_wrap(~site) + ggthemes::theme_tufte()

ggplot(data = df2, aes(time,temperature)) +
  geom_point(data = filter(df2, err == 19), aes(time, temperature, color = est_source), shape = 19) +
  geom_point(data = filter(df2, err == 1), aes(time, temperature, color = est_source), shape = 1) +
  geom_line(aes(time, mean_w)) +
  geom_ribbon(aes(time, ymin = lower_w, ymax = upper_w), 
                alpha = .3, fill = "royalblue") +
  geom_hline(yintercept = 0) +
  labs(color = paste(as.character(percent_error),"% error",sep=""), x = "Date", y = "Temperature") +
  ggthemes::theme_tufte() + 
