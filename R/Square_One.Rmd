...
create = function(A, AR, s_t, meanT, years = 1, tao, df = 3, sp_n=NA, snow=NA, source = "air"){
  ...
  if(source == "water"){
    ...
    i = ceiling((365*24 - sp_n) - 182.5*tao); sn = cos(2*pi*1:sp_n/sp_n)-1; r = (N/years)-i-length(sn)
    #Snow effect
...
generate = function(air_ends = F, air = F, years = 1, randomize = c(T,T), global){
  ...
  air_A = rnorm(1, .4*latitude, latitude*.02)/2 # Temperature range is latitude dependant.
  ...
  #Create Data.
  df = data.frame(
    air= create(A=air_A, AR=air_AR, s_t=5, meanT=air_meanT, years=years, tao=tao+0.07, df=4),
    water= create(A=water_A, AR=water_AR, s_t=20, meanT=water_meanT, years=years, tao=tao, df=10,
  ...
  cfs = data.frame(latitude, air_tao = tao-0.07, water_tao = tao,
...
multi_site = function(n = 1, years = 1, air_ends = F, air = F, randomize = c(T,T)){
  ...
  dat$daily = dat$daily %>% group_by(site) %>% mutate(year = lubridate::year(time),
                                                      d = yday(time - days(x=yday(time[1])-1))) %>% 
...
create_inits = function(K = 2, A_ij = c(.9,.1,.1,.9), air_A, air_mean, chains = 1){
  ...
  init = list(A_ij = matrix(A_ij, nrow = K, ncol = K),
              ...
              A = NA, alpha_w = if_else(air_mean>0,air_mean,2))
  init$A = if_else(init$alpha_w<13.5, init$alpha_w, (27-init$alpha_w))
...
df = multi_site(n = 20, years = 2, air_ends = T, air = F, randomize = c(F,T))
...
mod = sampling(compile, data= list(N = length(df$daily$temperature),
                                 ...
                                 water_mean = inits[[1]]$alpha_w,
                                 prior = 1),
               pars = c("alpha_w","A","tau_est","snow_w","alpha_tk","gamma"),
               init = inits, warmup = 500, iter = 1000, chains = 4, thin = 3,
               control = list(adapt_delta = .9, max_treedepth = 12), save_warmup = F)
...
A = data.frame(t(apply(cf$A, 2, `quantile`, probs = c(0.025,.25,0.5,.75,.975)))) %>% mutate(param = "A[w]")
global = df$coefs %>% select(water_meanT, water_A) %>% gather(key = obs_param, value = observed) %>% select(-1)
global = bind_rows(alpha,A) %>% rename(lower = X2.5., lower_mid = X25., fit = X50., upper_mid = X75., upper = X97.5.) %>% bind_cols(.,global) %>% mutate(prior = c(inits[[1]]$alpha_w, inits[[1]]$A))
...
fit = df$daily %>% mutate(row = 1:length(site)) %>% group_by(site,time) %>% do({
  ...
  water = cf$alpha_w[,.$site] + cf$A[,.$site]*cos(2*pi*.$d/.$n + cf$tau_est[,.$site,1])
  air = df$coefs$air_meanT[.$site] + df$coefs$air_A[.$site]*cos(2*pi*.$d/.$n + cf$tau_est[,.$site,2])
...
percent_error = fit %>% group_by(site) %>%
  do({
    ...
    error1 = round(sum(.$source[.$fp_fit>c]!="water")/n_AG*100)
    error2 = round(sum(.$source[.$fp_fit<c]=="water")/n_water*100)
