...
library(tidyverse); library(rstan); library(lubridate); library(tibble)
...
create = function(A, AR, s_t, meanT, years = 1, tao, df = 3, snow=NA, source = "air"){
  ...
  noise = arima.sim(model = list(ar = AR), n = N, sd = sd)
  ...
  if(source == "water"){
    ...
    #snow_adj = adj * snow
    snow_adj = snow*cos(2*pi*1:N/(N/2) + (tao+.5)*pi)
  }
...
generate = function(air_ends = F, air = F, years = 1, randomize = c(T,T), global){
 ...
 #.65 
  ...
  #Create Data.
  df = data.frame(
    air= create(A=air_A, AR=air_AR, s_t=5, meanT=air_meanT, years=years, tao=tao+0.035, df = 7),
...
### Functions that approximate values to constrain the search space. ###
...
n_init = function(time){
  ...
  if(length(time) == 1) cc = 365
    ...
    dur = data.frame(table(as.numeric(as.duration(time[2:length(time)]-time[1:(length(time)-1)])))) %>% 
      arrange(desc(Freq)) %>% as_tibble(.)
    dur = as.numeric(as.character(dur[[1]]))[1]
    cc = as.numeric(dyears(1)/dur[1])
  }
  if_else(leap_year(year(time)),cc+cc/365,cc)
}
...
create_inits = function(K = 2, A_ij = c(.9,.1,.1,.9), air_A, air_mean, chains = 1, sites = 1){
  ...
  init = list(A_ij = matrix(A_ij, nrow = K, ncol = K),
              ...
              tau_est = matrix(c(.85,.9),nrow = sites, ncol = 2,byrow = T))
...
prob_plot = function(mod, date, temperature, air = NULL, smooth = T, err_perc = NULL,
  ...
  #Add d and n values.
  if(dur == "hour") {
    df = df %>% mutate(day = ymd(floor_date(date, unit = "day")),
                       year = lubridate::year(date),
                       d = (yday(date)*24)+(hour(date)-24)+1) %>% 
      group_by(year) %>% mutate(n = n_init(date)*2)
    
    #Multiply n by 2 for observed data taken every other hour.
  }
  else {
  ...
  fit_w = data.frame(t(sapply(c(1:nrow(df)), function(x){
    quantile(cf$alpha_w + cf$A[,1,1] * cos(2*pi*df[x,"d"][[1]]/df[x,"n"][[1]] + cf$tau_est[,1,1]*pi) +
               cf$snow*cos(2*pi*df[x,"d"][[1]]/(df[x,"n"][[1]]/2) + (cf$tau_est[,1,1]+0.65)*pi),
  ...
  fit_a = data.frame(t(sapply(c(1:nrow(df)), function(x){
    quantile(bc_climate$air_mean + bc_climate$air_A * cos(2*pi*df[x,"d"][[1]]/df[x,"n"][[1]] + cf$tau_est[,1,2]*pi),
             probs = c(0.025,.25,0.5,.75,.975))
  }))) %>% 
  ...
  df = df %>% bind_cols(.,water,fit_w,fit_a) %>% 
    ...
    group_by(day) %>% mutate(prob_day = exp(mean(log(p_prob)))) %>% 
    ungroup() %>% mutate(est_source = if_else((p_prob<0.1 | prob_day<0.4),"air","water"))
  if(!is.null(err_perc)){
  ...
  #ggsave(filename = "probPlot.png", path = "~/Documents/rTDataScrub/images", plot = p, width = 4.25, height = 6.5, units = "in", device = "png", dpi = 500)
  list(p1p,p1,p1f)
}
```
...
#write_rds(df, "./stan/Mod_Results/single_sim_df.rds")
df = read_rds("./stan/Mod_Results/single_sim_df.rds")
inits = create_inits(air_A = df$coefs$air_A, air_mean = df$coefs$air_meanT, chains = 4)

#Compile and run model.
#compile = stan_model("./stan/02_HMM.stan")
#mod = sampling(compile, data= list(N = nrow(df$hourly), K = 2,
#                                  y = round(df$hourly$temperature,2),
#                                  d = df$hourly$d,
#                                  n = df$hourly$n,
...
#write_rds(mod, "./stan/Mod_Results/single_sim_500.rds")
mod = read_rds("./stan/Mod_Results/single_sim_500.rds")
#Plot
prob_plot(mod, df$hourly$time, df$hourly$temperature, smooth = F, err_perc = df$hourly$source,
          bc_climate = list(air_mean = df$coefs$water_meanT, air_A = df$coefs$air_A), dur = "hour")

prob_plot(mod, df$hourly$time, df$hourly$temperature, smooth = T, err_perc = df$hourly$source,
          bc_climate = list(air_mean = df$coefs$water_meanT, air_A = df$coefs$air_A), dur = "hour")
```
...
#  filter(site==49, year(date)==2014) %>% 
#  mutate(n = n_init(date), d = yday(date))

#Hourly
df = read_rds("./Data/thompson_hourly_water_manual_clean.rds") %>% 
  filter(site==49, year(date)==2015) %>% 
...
mod = sampling(compile, data= list(N = nrow(df), K = 2,
                                   ...
                                   precip = 1),
               pars = c("accumulator","annual","season","annual_gen","season_gen",
                        "unalpha_tk","logbeta","loggamma","beta"),
               include = FALSE, init = inits, iter = 500, chains = 4, thin = 1,
               control = list(adapt_delta = .8, max_treedepth = 10), save_warmup = T)
write_rds(mod, "./stan/Mod_Results/Hourly49HMM_300_obs.rds")
mod = read_rds("./stan/Mod_Results/Hourly49HMM_300_obs.rds")

shinystan::launch_shinystan(mod)

#Filter Plot
prob_plot(mod, df$date, df$temperature, air = air, smooth = F,
          bc_climate = list(air_mean = df_inits$air_mean, air_A = df_inits$air_A), dur = "hour")

#Smoothed Plot
p_single = prob_plot(mod, df$date, df$temperature, air = air, smooth = T, err_perc = df$error_lab,
          bc_climate = list(air_mean = df_inits$air_mean, air_A = df_inits$air_A), dur = "hour")
```
...
Now we can apply our HMM to simulated and raw stream temperature data at multiple sites in the North Thompson. Originally I was thinking this model would be hierarchical but found the global models pulled the site estimates away from the data resulting in poor state probability estimation. The only differenc with the model below is it allows for multiple sites and years to be fit rather than just a single site.


```{r Multi-Simulated HMM}
#Simulate
df = multi_site(n = 20, years = 1, air_ends = T, air = T, randomize = c(T,T))
#Hourly
df$hourly = df$hourly %>% group_by(site) %>% mutate(reset = if_else(time==min(time),1,
                                                  if_else(time==max(time),2,0)))
#Initial starting parameters
inits = create_inits(air_A = df$coefs$air_A, air_mean = df$coefs$air_meanT, chains = 4)
                                                                    
#Compile and run model.
compile = stan_model("./stan/04_HMM_MultiSite.stan")
mod = sampling(compile, data= list(N = length(df$hourly$temperature),
                                 S = length(unique(df$hourly$site)),
                                 site = df$hourly$site,
                                 R = length(unique(df$hourly$site)),
                                 rowe = df$hourly$site,
                                 K = 2,
                                 y = round(df$hourly$temperature,2),
                                 d = df$hourly$d_cycle,
                                 reset = df$hourly$reset,
                                 accum = which(df$hourly$reset==2),
                                 n = df$hourly$n,
                                 air_A = df$coefs$air_A,
                                 air_mean = df$coefs$air_meanT),
...
#write_rds(mod, "./stan/Mod_Results/HourlyMultiSite_500_sim.rds")


#Daily 
df$daily = df$daily %>% group_by(site) %>% mutate(reset = if_else(time==min(time),1,
                                                  if_else(time==max(time),2,0)))
#Compile and run model.
compile = stan_model("./stan/04_HMM_MultiSite.stan")
mod = sampling(compile, data= list(N = length(df$daily$temperature),
               ...
               init = inits, iter = 500, chains = 4,
               control = list(adapt_delta = .8, max_treedepth = 10))
#write_rds(mod, "./stan/Mod_Results/DailyMultiSite_500_sim.rds")
```
...
# 5.) sites = c(49,63,54,39,1,80,87,93,100,77)
...
mod = sampling(compile, data= list(N = length(df$temperature),
               ...
               pars = c("accumulator","annual","season","annual_gen","season_gen",
                        "unalpha_tk","logbeta","loggamma","beta"),
               include = FALSE, init = inits, iter = 100, chains = 4,
               control = list(adapt_delta = .8, max_treedepth = 10))
#write_rds(mod, "./stan/Mod_Results/HourlyMultiSite_500_obs.rds")

#shinystan::launch_shinystan(mod)
```

```{r Thompson Watershed Organize Results}
#Read in model.
mod = read_rds(path = "./stan/Mod_Results/HourlyMultiSite_500_obs.rds")
...
full = df %>% mutate(filter = apply(cf$alpha_tk[,,1],2,FUN = quantile, probs = .5),
                     smooth = apply(cf$gamma[,,1],2,FUN = quantile, probs = .5)) %>%
  group_by(site) %>%
	do({
...
rtrn = full %>% group_by(site, year_no, day) %>% do({
  r = unique(.$rowe); d = unique(.$d); n_cyc = unique(.$n); id = unique(.$new_id)
	water_est = (cf$alpha_w[,r] + cf$A[,r,1]*cos(2*pi*d/n_cyc + cf$tau_est[,id,1]*pi)) + cf$snow[,r]*cos(2*pi*d/(n_cyc/2) + (cf$tau_est[,id,1]+0.65)*pi)
	air_est = df_inits[r,"air_mean"] + cf$A[,r,2]*cos(2*pi*d/n_cyc + cf$tau_est[,id,1]*pi)
	if(any(is.na(water_est))) browser()
	water = quantile(water_est, probs = .5)
	air = quantile(air_est, probs = .5) 
	std_w = quantile(cf$sigma[,id,1], probs = .5)
	std_a = quantile(cf$sigma[,id,2], probs = .5)
	data.frame(water,air,std_w,std_a)
})

#Correct names
names(rtrn) = c("site","year_no","day","water","air","std_w","std_a")
rtrn = left_join(full,rtrn)
#rm(list = c("full","test","dates_full"))
...
rtrn = left_join(rtrn,air_stream) %>% left_join(.,air_obs, by = c("air_site"="place","day"="date"))

#Summarize by day the source expectation
test = rtrn %>% group_by(site, logger, day) %>% 
  mutate(prob_day = exp(mean(log(smooth), na.rm = T))) %>% ungroup() %>% 
  mutate(est_source = if_else((smooth<1e-2 | prob_day<1e-9),"air","water"))

#Sequence of probability cut points
cut = vector(mode='numeric',length=15); cut[1] = .1
for(i in 1:15){
  #browser()
  cut[i+1] = 0.1/10^i
}
cut_p = data.frame(cut = cut)

#Error rates at varied cut points.
cut_pts = cut_p %>% group_by(cut) %>% do({
  #browser()
  c = .$cut
  rtrn %>% group_by(site,logger,day) %>% 
    mutate(prob_day = exp(mean(log(smooth), na.rm = T))) %>% ungroup() %>% 
    mutate(est_source = if_else((smooth<c | prob_day<c),"air","water")) %>% 
    filter(!is.na(error_lab)) %>% 
    summarise(error_n = sum(error_lab!=est_source),
              error_perc = 100*round(error_n/n(),2),
              typeI = sum(est_source=="water" & error_lab!="water")/n(),
              typeII = sum(est_source=="air" & error_lab=="water")/n())
})

#Overal error rate
test %>% filter(!is.na(est_source)) %>% ungroup() %>% 
  summarise(error_n = sum(error_lab!=est_source),
            n = n(),
            air = sum(error_lab =="air")/n,
            error_perc = 100*round(error_n/n,2),
            typeI = sum(est_source=="water" & error_lab!="water")/error_n,
            typeII = sum(est_source=="air" & error_lab=="water")/error_n)

#Summarize by site the error rate
percent_error = test %>% filter(!is.na(est_source)) %>% group_by(site) %>% 
  summarise(error_rate = 100*round(sum(error_lab!=est_source)/n(),2))
#Plot Labels
percent_error$label = paste(as.character(percent_error$site)," (", as.character(percent_error$error_rate),"%)", sep = "")

#Yearly summary
test %>% filter(!is.na(est_source)) %>% group_by(site, year) %>%
  summarise(error_rate = 100*round(sum(error_lab!=est_source)/n(),2)) %>% 
  arrange(error_rate) %>% print.data.frame()


```

```{r Thompson Watershed Plot Results}
#Plot error rates
p1 = ggplot(cut_pts) + 
  geom_line(data = cut_pts %>% gather(key = error_type, value = percent_error, typeI, typeII),
            aes(log(cut, base = 10), percent_error*100, color = error_type), linetype = 3) +
  geom_line(aes(log(cut, base = 10),error_perc), linetype = 2) + 
  scale_x_reverse(breaks = c(-4,-8,-12,-16), labels = c("1e-4","1e-8","1e-12","1e-16")) +
  #geom_hline(yintercept = 4.33, size = .1) +
  scale_color_manual(values = c("#45062e","#2b6aff"), breaks = c("typeI","typeII"),
                     labels = c("type I","type II"), name = "Disagreement 'Error' Type") +
  ggthemes::theme_tufte() +
  ...
  guides(colour = guide_legend(title.position = "top")) +
  labs(y = "Percent Disagreement", 
       x = "Water Break-Point (%)")
ggsave(filename = "Error_Type_Obs.png", path = "~/Documents/rTDataScrub/images", plot = p1, width = 4, height = 3.5, units = "in", device = "png")

#Colorbar
ref = data.frame(ref=seq(0,1,by=0.01), col = viridis::viridis(100+1, direction = -1, alpha = 0.1))
col = unlist(apply(test,1,function(x){
  as.character(ref[round(ref$ref,2)==round(as.numeric(x["smooth"][[1]]),2),"col"][1])
}))
col = col[!is.na(col)]

p = ggplot(test) +
  geom_point(data = test %>% ungroup %>% filter(!is.na(temperature)),
             aes(day, 40), color = col, shape = 19, size = .3) +
  geom_point(data=test %>% ungroup %>% select(site,day,air_temperature) %>% distinct(),
             aes(day, air_temperature),color = "red", alpha = .1, shape = 1, size = .3) +
  geom_ribbon(aes(x=day,ymin=air-1.97*std_a,ymax=air+1.97*std_a), alpha = .1, fill = "#7eb795") +
  geom_ribbon(aes(x=day,ymin=water-1.97*std_w,ymax=water+1.97*std_w), alpha = .1, fill = "#5b0982") +
  geom_point(data = test %>% filter(!is.na(temperature)),
             aes(day, temperature, color = est_source), shape = 19, size = .3) +
  geom_hline(yintercept = 0, size = .2, linetype = 3) +
  annotate("segment", x=min(test$day), xend = max(test$day), y=-Inf, yend=-Inf, size = .2) +
  scale_color_manual(values = c("water" = "#2b6aff", "air" = "#ffc021")) +
  ggthemes::theme_tufte() +
  scale_x_date(date_breaks = "6 month", date_labels = "%y-%m") +
  labs(color ="Estimated Thermal Source", x = "Year-Month",
       y = expression("Temperature ("*degree*"C)")) +
  theme(legend.direction = "horizontal", legend.position = c(.77,1.07),
        plot.margin = unit(c(4,1,1,1),"lines"),
        strip.text = element_blank(),
        axis.text = element_text(size = 8)
  ) +
  guides(color = guide_legend(title.position="top", title.hjust = 0.8))+
  facet_wrap(nrow = 5, ncol = 2, facets = ~site) +
  geom_text(data = percent_error, aes(x = min(test$day), y = 40, label = label),
            hjust = "left", size = 2.3)
...
ggsave(filename = "Probs_Obs_Full.png", path = "~/Documents/rTDataScrub/images", plot = p_full, width = 13, height = 8.5, units = "in",  device = "png", dpi = 500)
```

```{r Defense Talk}
single = test %>% ungroup() %>% filter(site == 49, !is.na(temperature), year == 2015)
p1 = ggplot(single) +
  geom_point(aes(day, temperature), colour = "#2b6aff", size = .1) +
  geom_hline(yintercept = 0, size = .2, linetype = 3, colour = "white") +
...
ggsave(filename = "temp_examp.png", path = "~/sfuvault/Thesis/Defense_Talk/", plot = p1, width = 6.5, height = 5, units = "in", device = "png", bg = "transparent") 

p2 = ggplot(single) +
  #geom_point(aes(day, temperature), colour = "#2b6aff", size = .1) +
  geom_point(aes(day, temperature, color = est_source), size = .1) +
  scale_color_manual(values = c("water" = "#2b6aff", "air" = "#ffc021")) +
  geom_line(aes(day, air), color = "#EE220C") +
  geom_line(aes(day, water), color = "#00A2FF") +
  geom_hline(yintercept = 0, size = .2, linetype = 3, colour = "white") +
  labs(x = "Year-Month", y = expression("Temperature ("*degree*"C)")) +
  ylim(-6, 33)+
  ggthemes::theme_tufte() +
  theme(legend.position = 'none',
        ...
        plot.background = element_rect(colour = NA, fill = "transparent"),
        panel.background = element_rect(colour = NA, fill = "transparent"))
ggsave(filename = "temp_examp3.png", path = "~/sfuvault/Thesis/Defense_Talk/", plot = p2, width = 6.5, height = 5, units = "in", device = "png", bg = "transparent") 

single = single %>% mutate(SE_Amin = if_else(air-1.97*std_a>-6,air-1.97*std_a,-6),
                  SE_Amax = if_else(air+1.97*std_a<33,air+1.97*std_a,33),
                  SE_Wmin = if_else(water-1.97*std_w>-6,water-1.97*std_w,-6),
                  SE_Wmax = if_else(water+1.97*std_w<33,water+1.97*std_w,33))
p3 = ggplot(single) +
  geom_ribbon(aes(x=day,ymin=SE_Amin,ymax=SE_Amax), alpha = .1, fill = "#FF644E") +
  geom_ribbon(aes(x=day,ymin=SE_Wmin,ymax=SE_Wmax), alpha = .1, fill = "#73FDEA") +
  #geom_point(aes(day, temperature), colour = "#2b6aff", size = .1) +
  geom_point(aes(day, temperature, color = est_source), size = .1) +
  scale_color_manual(values = c("water" = "#2b6aff", "air" = "#ffc021")) +
  geom_line(aes(day, air), color = "#EE220C") +
  ...
  theme(legend.position = 'none',
        ...
        plot.background = element_rect(colour = NA, fill = "transparent"),
        panel.background = element_rect(colour = NA, fill = "transparent"))
ggsave(filename = "temp_examp4.png", path = "~/sfuvault/Thesis/Defense_Talk/", plot = p3, width = 6.5, height = 5, units = "in", device = "png", bg = "transparent") 
...
After a mean and amplitude are identified across sites we should be able to standardize the data by subtracting the mean and dividing by the standard deviation. These z-scores would suggest that anything greater than 3 or -3 is beyond the 99th percentile and unlikely. This will only work if the standard deviation is allowed to change seasonally. Alternatively we could only normalize the data by subtracting the mean and then ask which data are extreme relatively all data across sites for that given day. This normalizing option may account for when the model doesn't capture a common components across sites.
