...
df = multi_site(n = 20, years = 1, air_ends = T, air = F, ground_sat = F, randomize = c(F,T,F))
ggplot(df$daily, aes(time, temperature, color = source)) + geom_point() + facet_wrap(~site)
...
#Compile and run model.
compile = stan_model("./stan/HMM_GWA.stan")
mod = sampling(compile, data= list(N = length(df$daily$temperature),
                                 S = length(unique(df$daily$site)),
                                 site = df$daily$site,
                                 K = 2,
...
percent_error = df2 %>% group_by(site) %>%
  do({
    data.frame(
      ...
      error2 = round((1-sum(.$source[.$source == "water"] == .$est_source[.$source == "water"])/sum(.$source == "water"))*100),
      error1 = round((sum(.$source[.$est_source=="water"]!="water")/sum(.$source!="water"))*100),
      x_pos = min(df2$time),
      y_pos = max(df2$temperature)
    )
  }) %>% 
  mutate(label = paste("type I: ",as.character(error1),"% of ",as.character(n_water)," -- type II: ",as.character(error2),"% of ",as.character(n_AG),sep=""))

df2[df2$median<.5,"est_source"] = "uncertain"
...
p = ggplot(data = df2, aes(time,temperature)) +
  ...
  geom_point(aes(color = est_source), size = 0.5) +
  geom_line(aes(time, mean_w), size = .3) +
  geom_line(aes(time, mean_a), size = .3) +
  geom_rug(data = filter(df2, err == 19), aes(time, temperature), sides = "b") +
  scale_color_manual(values = c("water" = "#2b6aff", "air" = "#ffc021", "unknown" = "#45062e")) +
