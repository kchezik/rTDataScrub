...
create = function(A, AR, s_t=c(0,0), meanT, years = 1, tao){
  ...
  sd = (d_cycle+2)*s_t[1] # Changing standard deviation by season.
...
prob_plot = function(mod, date, temperature, smooth = T, err_perc = NULL){
  ...
  #Coefficients
  cf = broom::tidy(mod, pars = c("alpha_water","alpha_air","A","tau_est","sigma","sigma_g")) %>%
  ...
  #Add Model Fits
  df2$water_fit = cf$`alpha_water` + cf$`A[1]`*cos(2*pi*df2$day/nrow(df2) + cf$`tau_est[1]`*pi)
  ...
  p1f = p1f + geom_line(aes(date, water_fit), size = .3) +
    ...
    ggthemes::theme_tufte() +
    theme(legend.direction = "horizontal", legend.position = c(x_pos, .85),
      axis.line.x = element_line(color = "black", size = .2),
      strip.text = element_text(size=8), axis.text = element_text(size = 8), 
      axis.title = element_text(size = 9), legend.title = element_text(size = 8),
  ...
  p = gridExtra::grid.arrange(p1p, p1, p1f, layout_matrix = matrix(data = c(1,2,3), nrow = 3, ncol = 1))
  ggsave(filename = "probPlot.pdf", path = "~/Documents/rTDataScrub/images", plot = p, width = 7.5, height = 4.5, units = "in", device = "pdf")
}
```
...
write_rds(mod, path = "~/Documents/rTDataScrub/stan/Mod_Results/HMM_mod_1000_sim.rds")
broom::tidy(mod, pars = c("A","alpha_water","alpha_air","tau_est","sigma","sigma_g","A_ij"), "median")
#Plot
...
inits = create_inits(K = 3, air_A = 23, air_alpha = 5, chains = 4, n_sites = 1)
...
write_rds(mod, path = "~/Documents/rTDataScrub/stan/Mod_Results/HMM_mod_1000_obs.rds")
...
write_rds(mod, path = "~/Documents/rTDataScrub/stan/Mod_Results/HMM_smooth_mod_1000_obs.rds")
...
#Observed
temp = left_join(df$daily, df$coefs, by = "site")
temp = temp %>% group_by(site) %>%  mutate(mean_w = water_meanT + water_A * cos(2*pi*d/365+tau_approx*pi),
                                           mean_a = air_meanT + air_A * cos(2*pi*d/365+tau_approx*pi))
temp %>% group_by(site) %>% arrange(time) %>% 
ggplot(., aes(time, temperature)) + geom_point(aes(col = source),alpha=.2) + 
  geom_line(aes(time, mean_w), color = "blue") +
  geom_line(aes(time, mean_a), color = "red") +
  facet_wrap(~site)
...
percent_error = df2 %>% group_by(site) %>%
  do({
    ...
    error1 = round(sum(.$source[.$est_source=="water"]!="water")/n_AG*100)
    error2 = round(sum(.$source[.$est_source!="water"]=="water")/n_water*100)
...
df2[df2$median<.95,"est_source"] = "uncertain"
