...
prob_plot = function(mod, date, temperature, air = NULL, smooth = T, err_perc = NULL,
  ...
  fit_w = data.frame(t(sapply(c(1:nrow(df)), function(x){
    quantile(cf$alpha_w + cf$A[,1] * cos(2*pi*df[x,"d"][[1]]/df[x,"n"][[1]] + cf$tau_est[,1]*pi) +
               cf$snow*cos(2*pi*df[x,"d"][[1]]/(df[x,"n"][[1]]/2) + (cf$tau_est[,1]+0.65)*pi),
...
mod = sampling(compile, data= list(N = nrow(df), K = 2,
               ...
               include = FALSE, init = inits, iter = 100, chains = 4, thin = 1,
               control = list(adapt_delta = .8, max_treedepth = 10), save_warmup = T)
...
write_rds(mod, "./stan/HourlyMultiSite_500_sim.rds")
...
mod = sampling(compile, data= list(N = length(df$daily$temperature),
               ...
               init = inits, iter = 500, chains = 4,
               control = list(adapt_delta = .8, max_treedepth = 10))
write_rds(mod, "./stan/DailyMultiSite_500_sim.rds")
```
...
# sites = c(49,63,54,39,1,80,87,93,100,77)
#Hourly
df = read_rds("./Data/thompson_water.rds") %>% 
  filter(site == 77) %>% 
...
inits = create_inits(air_A = df_inits$air_A, air_mean = df_inits$air_mean, chains = 4, sites = 1)
...
mod = sampling(compile, data= list(N = length(df$temperature),
               ...
               pars = c("accumulator","annual","season","annual_gen","season_gen",
                        "unalpha_tk","logbeta","loggamma","beta"),
               include = FALSE, init = inits, iter = 500, chains = 4,
               control = list(adapt_delta = .8, max_treedepth = 10))
write_rds(mod, "./stan/HourlyMultiSite_500_obs77.rds")

shinystan::launch_shinystan(mod)

ggplot(df,aes(date, temperature, color = as.factor(rowe))) + geom_point() + facet_wrap(~site)


  #Read in model.
mod_d = read_rds(path = "./stan/Mod_Results/MultiSite_500_obs.rds")
