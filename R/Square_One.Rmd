...
create_inits = function(K = 3, A_ij = c(.9,.1,.1,.1,.9,.1,.1,.1,.9), p_1k = c(.495,.495,.01),
  ...
  init = list(p_1k = matrix(data = p_1k, nrow = n_sites, ncol = K, byrow = T),
              ...
              A = matrix(data = c(rep(4, n_sites),air_A), nrow = n_sites, ncol = 2),
              alpha_air = air_alpha,
...
prob_plot = function(mod, date, temperature, smooth = T, err_perc = NULL){
  ...
  #Coefficients
  cf = broom::tidy(mod, pars = c("alpha_w","alpha_a","A","tau_est","sigma","sigma_g")) %>%
  ...
  #Add Model Fits
  df2$water_fit = cf$`alpha_w` + cf$`A[1]`*cos(2*pi*df2$day/nrow(df2) + cf$`tau_est[1]`*pi)
...
The model fits the real water and air temperature data well and capture the major differences between the two temperature sources. This is encouraging that the models, though the same, may have such different parameter values they will be seperable in the HMM.
...
inits = list(list(p_1k=c(.495,.495,.01), A = c(15,25), alpha_a = 10, alpha_w = c(4,5)),
             list(p_1k=c(.495,.495,.01), A = c(15,25), alpha_a = 10, alpha_w = c(4,5)),
             list(p_1k=c(.495,.495,.01), A = c(15,25), alpha_a = 10, alpha_w = c(4,5)),
             list(p_1k=c(.495,.495,.01), A = c(15,25), alpha_a = 10, alpha_w = c(4,5)))
#Compile Model
compile = stan_model("./stan/HMM.stan")
...
df = multi_site(n = 20, years = 1, air_ends = T, air = F, ground_sat = F, randomize = c(F,T,T))
...
cf2 = broom::tidy(mod, pars = c("alpha_a","alpha_w","sigma_g"), "mean") %>%
  select(-std.error) %>% 
  mutate(site = as.numeric(stringr::str_extract_all(string = term, pattern = "\\d+(?=\\])",simplify = T)),
         model = c(rep(2,20),rep(1,40)),
         coef = stringr::str_extract_all(term, "\\w+", simplify = T)[,1]) %>% 
  select(-term)

#cf1[cf1$model==1 & cf1$coef=="alpha_w",c("model","coef")] = c(rep(3,20), rep("t0",20))
#cf1[cf1$model==2 & cf1$coef=="alpha_w",c("model","coef")] = c(rep(1,20), rep("alpha",20))
#cf2[cf2$coef=="alpha_a","coef"] = "alpha"

cf = rbind(cf1,cf2) %>% spread(key = coef, value = estimate)

#Mean water temperature real vs. estimate
plot(df$coefs$water_meanT, cf[cf$model == 1,"alpha_w"], xlim = c(0,10), ylim = c(0,10))
...
temp = temp %>% mutate(
  ...
  lower_g = mean_g - 1.96*sigma_g) %>% select(-c(6:12))
...
percent_error = df2 %>% group_by(site) %>%
  do({
    ...
    data.frame(
      ...
      error1 = round((1-sum(.$source[.$source == "water"] == .$est_source[.$source == "water"])/sum(.$source == "water"))*100),
      error2 = round((sum(.$source[.$est_source=="water"]!="water")/sum(.$source!="water"))*100),
      x_pos = min(.$time[lubridate::month(.$time)==2])-30
    )
  })

percent_error$y_pos = if_else(max(df2$temperature)%%10!=0,
                            round(max(df2$temperature)/10)*10-10,
                            max(df2$temperature))
percent_error = percent_error %>%
  mutate(label1 = paste("type I : ",as.character(error1),"% of ",as.character(n_water),sep = ""),
         label2 = paste("type II: ",as.character(error2),"% of ",as.character(n_AG),sep = ""))
...
p = ggplot(data = df2, aes(time,temperature)) +
  ...
  geom_ribbon(aes(time, ymin = lower_w, ymax = upper_w), 
                alpha = .3, fill = "royalblue") +
  geom_hline(yintercept = 0) +
  labs(color = "Estimated Thermal Source", x = "Date", y = "Temperature") +
  ggthemes::theme_tufte() + 
  theme(legend.direction = "horizontal", legend.position = "top") + 
  guides(color = guide_legend(title.position = "top")) +
  facet_wrap(~site)
p + geom_text(data = percent_error, aes(x = x_pos, y = y_pos, label = label1), hjust = "left") +
  geom_text(data = percent_error, aes(x = x_pos, y = y_pos-8, label = label2), hjust = "left")
```

After much tinkering we have chosen to linearize the global model described in the water only random effect model (i.e.,"water_rand.stan"). The model describing the steapness of the relationship seemed much too sensative, biasing mean water temperature estimates in the lower exponential part of the sigmoidal curve. We also included an amplitude global model that is approximately linear with the mean water temperature estimates. This model works for northern climate locaitons but I suspect will fail as we get closer to the equator and water temperature never near 0ºC but rather are upper bound. This aspect of the model will be left for future development at this time. I've also discovered the importance of narrow priors on air and ground models and looser priors on water model parameters. By doing this we put the focus on estimating water parameters and less weight on partially known values such as air parameters. We expect that the water data this model receives will mostly be water with limited air and often no ground temperature data. As such we want to make sure the data are primarily being used to inform the water model and less so the ground and air models.
