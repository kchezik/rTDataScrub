...
multi_site = function(n = 1, years = 1, air_ends = F, air = F, randomize = c(T,T)){
  ...
  dat$daily = dat$daily %>% group_by(site) %>% 
    mutate(year = lubridate::year(time), d = yday(time - days(x=yday(time[1])-1)),
           reset = if_else(time==min(time),1,if_else(time==max(time),2,0))) %>% 
...
create_inits = function(K = 2, A_ij = c(.9,.1,.1,.9), air_A, air_mean, tau, chains = 1){
  #Create single chain list
  A_ij = diag(K); A_ij = if_else(A_ij ==1, .9, .1/(K-1))
  init = list(A_ij = matrix(A_ij, nrow = K, ncol = K),
              b_alpha_w = 1, m_alpha_w = .1,
              tau_est = matrix(data = c(tau,tau+0.02), nrow=length(air_A) , ncol=2),
...
stream_snow = function(n, d, tau){
  rate = -sin(2*pi*d/n + tau*pi)
  day = if_else(rate>0, ((asin(-1*rate)-pi)*n)/(2*pi),0)
  if_else(day!=0,sin(2*pi*day/n+pi),0)
}
```
...
df = multi_site(n = 1, years = 2, air_ends = T, air = T, randomize = c(F,F))
inits = create_inits(air_A = df$coefs$air_A, air_mean = df$coefs$air_meanT,
                     tau = c(df$coefs$tau_approx, df$coefs$tau_approx+0.02), chains = 4)
...
mod = sampling(compile, data= list(N = nrow(df$daily), K = 2,
               ...
               pars = c("alpha_w","A","tau_est","snow_w","sigma","alpha_tk"),
               init = inits, warmup = 500, iter = 1000, chains = 4, thin = 2,
...
inits = create_inits(air_A = df_inits$air_A, air_mean = df_inits$air_mean,
                     tau = c(tau_init(time = df_T$day),tau_init(time = df_T$day)+0.02), chains = 4)
...
#Plot
p_single = prob_plot(mod, df_T$day, df_T$temperature, air = air, smooth = F,
...
df_dT_orig = df_T %>% mutate(year = lubridate::year(date), 
                        day = lubridate::floor_date(date, unit = "day"), 
                        doy = lubridate::yday(date)) %>%
  group_by(site, year, day, doy) %>% summarise(temperature = mean(temperature)) %>% 
  left_join(., ref) %>% group_by(site) %>% 
  mutate(d = yday(day - days(x=yday(day[1])-1)),
         reset = if_else(day==min(day),1,if_else(day==max(day),2,0))) %>% 
  group_by(site, year) %>% mutate(n_cycle = n_init(day))

#Remove temperatures less than 1.
df_dT = df_dT_orig %>% filter(temperature>1 | reset>0)

# add flag noting data gaps for forward and backward smoothing
logic = (diff(df_dT$day)>1 & df_dT$reset[-1] == 0)
df_dT[c(FALSE,logic),"reset"] = 1
...
mod = sampling(compile, data= list(N = length(df_dT$temperature),
               ...
               pars = c("b_alpha_w","m_alpha_w",
                        "alpha_w","A","tau_est","snow_w","sigma",
                        "alpha_tk","gamma"),
               init = inits, iter = 100, chains = 4, thin = 1,
               control = list(adapt_delta = .8, max_treedepth = 10),
               save_warmup = T, sample_file = "./stan/Mod_Results/04_HMM_Global_Samples.csv")

shinystan::launch_shinystan(mod)

#Read in model.
mod = read_rds(path = "./stan/Mod_Results/04_HMM_Global_500_Obs.rds")
