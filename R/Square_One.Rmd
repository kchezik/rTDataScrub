...
prob_plot = function(mod, date, temperature, n, air = NULL, smooth = T, err_perc = NULL){
  ...
  p1 = ggplot(df1) + 
    ...
    geom_point(aes(date, temperature, col = median), size = .3, shape = 19) +
  ...
  p1f = p1f + geom_point(aes(color = est_source), size = 0.3, shape = 19) +
    geom_line(aes(date, water_fit), size = .3) +
...
inits = create_inits(K = 2, air_A = df$coefs$air_A, air_alpha = df$coefs$air_meanT, chains = 4, n_sites = 1)
...
df_T = read_rds("./Data/thompson_water.rds") %>% filter(site != 102)
...
mod = sampling(compile, data= list(N = length(df_dT$temperature),
                                 ...
                                 water_mean = inits[[1]]$alpha_water,
                                 prior = 1),
               pars = c("b_alpha_w","m_alpha_w","alpha_a","alpha_w","A","tau_est","alpha_tk","gamma"),
               init = inits, iter = 500, chains = 4, thin = 3,
               control = list(adapt_delta = .9, max_treedepth = 12),
               save_warmup = F, sample_file = "04_HMM_GWA_Samples.csv")
...
cf1 = broom::tidy(mod, pars = c("A","sigma"), "median") %>%
  select(-std.error) %>% 
  mutate(
...
cf2 = broom::tidy(mod, pars = c("tau_est"), "median") %>%
  select(-std.error) %>% 
  mutate(
    new_id = as.numeric(stringr::str_extract_all(string = term, pattern = "\\d+",simplify = T)[,1]),
    model = c(rep(2,102),rep(1,102)),
    coef = stringr::str_extract_all(term, "\\w+", simplify = T)[,1]
  ) %>% 
  select(-term)

cf3 = broom::tidy(mod, pars = c("alpha_a","alpha_w"), "median") %>%
  select(-std.error) %>% 
...
cf = rbind(cf1,cf2,cf3) %>% spread(key = coef, value = estimate)
...
global =data.frame(new_id = c(cf[cf$model==1,"new_id"],cf[cf$model==2,"new_id"]),
                   ...
                   parameter = c(rep("alpha",102),rep("A",102)),
                   air_prior = c(inits[[1]]$alpha_air,inits[[1]]$A[,2]),
                   water_prior = c(inits[[1]]$alpha_water,inits[[1]]$A[,1])) %>% 
  filter(new_id != 102) #Removed because site is an outlier and extremely lake effected.
...
gp1 = ggplot() +
  ...
  geom_point(data = global %>% filter(parameter == "alpha"), aes(air, water), 
