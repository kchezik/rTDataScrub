...
where $\gamma$ is the number of observations per cycle and $d_{t}$ the integer location of observation $t$ in the cycle. We include a seasonal adjustment ($\tau$) to shift the cosine curve from the peak value of 1 at the origin to coincide with the point in the curve described by the first of January which is typically approximated by an adjustment of $\pi$ which begins the curve at it's lowest point.

The variance around the mean temperature is a distinguishing feature between annual air and water temperature cycles. Here we assume the variance is normally distributed in both states and captured in sigma. We do not expect errors to always be independant and identically distributed as auto-correlation between data-points at the sub-daily level will not be captured by the annual or seasonal models. As long as the primary components of the two states are well described, this compromise of normality assumptions shouldn't impact the efficacy of state estimation.
...
We expect $A_{k}$, $\tau_{k}$ (eq.$\ref{eq4}$) and $\sigma_{k}$ (eq.$\ref{eq6}$) to be ordered (i.e., $w \prec a$) because the higher thermal capacity of water dampens the response time to seasonal shifts in solar radiation and reduces the amplitude and variance of water's annual temperature cycle. Therefore, we expect the seasonality of water temperatures to lag behind air and express depressed amplitude and variance relative to the air temperature state. Ordering of variables not only captures the differential dynamics of the states but increases the stability of estimation by reducing the potential for state inversion (i.e., label switching) during the multi-chain MCMC process.

Although neither air or stream temperature profiles are perfectly described by a cosine curve, stream temperatures are particularly prone to hysteresis where spring and fall temperatures are not symetrical around summer's peak temperatures [@Letcher:2016]. Hysteresis in the annual stream temperature curve occurs for many reasons but most common is snow melt and rain. In northern latitudes spring snow melt depresses stream temperatures initially but waines in the summer leading to low flows and warmer stream temperatures only to return to a cooler state as fall rains increase river discharge. To allow our stream temperature curves a modecum of seasonal flexibility, we include a double cycle cosine curve that modulates the annual curve (eq. $\ref{eq4}$), 

\begin{linenomath*}
\begin{equation}
	\mathrm{season} = \phi*\cos(2\pi d_{1:\gamma}/(\gamma/2) + \tau_{w}\pi) \label{eq7}
\end{equation}
\end{linenomath*}

such that winter and summer temperatures are elevated, while spring and fall temperatures are depressed. The season effect is allowed to be shifted by a $\tau$ value in order to capture variation in climate over space. We also include a seasonal expansion factor ($\phi$) that describes the strength of the seasonal effect where zero suggests no effect of season. We limited this effect to be no greater than 3 degrees so as to limit its flexibility and avoid 'reaching' by the model to capture seasonally correlated air temperature readings.

*Temperature HMM*

By evaluating the likelihood of the data ($y_{t}$) given the state models (eq.$\ref{eq4}$), weighted by the previous time step's state probabilities (eq. $\ref{eq1}$), we can make inference about which state ($k=a,w$) the data are in at each time point. Iteratively evaluating eq. $\ref{eq3}$ returns state probabilities that indicate whether the temperature data represent an air or water source as well as our certainty of that estimate. We implimented this process in the probabilistic programming language Stan [@Stan:2017] with one modification. Given water temperature is the desired state, we expect most datasets will have limited air temperature records likely leading to the inaccurate coincidence of state parameter estimates during the fitting process. Furthermore, tools are currently available to approximate $\alpha_{a}$ and $A_{a}$ with relatively high accuracy over space and time [e.g., @Wang:2012]. Therefore, to avoid coincidence in state parameter estimates which would result in unclear state probabilities, we provide the air state model with known and modelled $\alpha$ and $A$ estimates for simulated and observed data respectivley. Priors indicating our estimated parameter value expectations were also incorporated into the model thereby providing coefficient guidance to states with limited data at a given site (see *Model Priors and Known Quantities* for values).

*Simulated Data*

We simulated data from our state to demonstrate model efficacy and test error rates under a variety of contrived but realistic scenarios. Using relationships similar to those described in Fig. 6 of @Gates:1999 and in @Pearce:1990 describing the effect of latitude ($L_{N}$) on mean annual air temperature ($\alpha_{a}$) and annual temperature range ($A_{a}$), we calculated $\alpha_{a}$ and $A_{a}$ for a variety of latitudes between 30 and 60$\text{\textdegree}$N.

\begin{linenomath*}
\begin{equation}
	\alpha_{a} = 27-(L_{N}-16)*0.57, \quad  A_{a}=L_{N}*0.45 \label{eq10}
\end{equation}
\end{linenomath*}
...
To demonstrate a real world application, we applied our HMM to raw stream temperature data collected at 8 sites in the Thompson River basin in central British Columbia Canada (Fig. $\ref{fig:6}$). Collected for the purpose of understanding geomorphic impacts on stream temperature, these data are not paired with air temperature data and represent a real world example of the challenges surrounding stream temperature data cleaning. These stream temperature data were collected between July of 2014 and August of 2017 and contain examples of all aforementioned errors. Although unconfirmed, we know errors are likely due to evidence upon retrieval but the exact location of those errors in the time series is unknown. To compare HMM and traditional methods of data cleaning we manually inspected and labelled data as either air or water based on prior knowledge and personal expertise. Eighteen weather stations owned and operated by Environment Canada (Fig. $\ref{fig:6}$), collecting air temperatures during the same period, were consolidated to represent eleven regions in the Thompson River basin and used as a visual reference to compare and contrast modeled source estimates with direct observations.
...
We expect temperature data in a given state, are more likely to remain in that state than transition to another state. Therefore, we applied weakly informative beta priors with shape parameters $alpha$ = 10 and $beta$ = 2 (median=0.85) for the probability of remaining in the current state and the inverse for the probability of transitioning between states. Moderately informative priors and fixed values were applied to some parameters where more is known about their values. By extracting mean annual air temperatures and associated annual temperature range estimates from the climate tool ClimateBC [@Wang:2012], we were able to provide locally adjusted (e.g., elevation, latitude) $\alpha_{a}$ and $A_{a}$ parameter estimates (known under simulation). We used ClimateBC air parameter estimates to provide the mean water temperature $\alpha_{w}$ and amplitude $A_{w}$ parameters moderately strong priors ($\sigma$ = 5). Due to our knowledge of seasonal cycles we estimated $\tau$ for the air and water state by assuming the lowest mean annual temperatures occur around the first of the year which coincide with a cosine curve adjustment of $\pi$.

We were able to directly calculated the parameter values in eq.$\ref{eq5}$ because temperature cycles occur at regular intervals. Each year typically contains 365 days with 366 in leap years. Thus our $\gamma$ value is typically some multiplication of 365 depending on the temperature sampling frequency. In this study we collected data every other hour (4380 obsevations per cycle), but also averaged the data to the daily level (360 observations per cycle), as summarized data are common in many temperature databases. Each observations $d$ value was calculated as the day or hour of the year starting with January-01 00:00:00 (M-D HH:MM:SS).

*Summary Statistic Calculations*

The simulted data offer the advantage of having known and estimated source values from which we can evaluate the efficacy of our HMM. Here we calculate how often our model estimates data to be water when the true source is air (i.e., type I error), and air when its true source is water (i.e., type II error), both as a percentage of the number of known air or water temperature data respectively. Much like in $p$-values, these error types are subject to an arbitrary certainty boundary set by our desire to commit one type of error over the other. Our HMM requires the probability estimates between air and water states sum to 1 for each observation, thus whichever state has greater than 50% support determines the temperature source for the given observation. Alternatively, we can choose much more stringent cutoffs if we prefer to be more certain of the temperatures source. For instance, we can set our certainty at 90% before accepting data as sourced from water which would reduce type I errors but likley increase type II errors. Here we calculate the error rates as a function of several certainty boundaries to demonstrate changes in error type.
