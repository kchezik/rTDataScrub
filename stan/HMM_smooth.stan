    ...
    for (tforward in 0:(N-2)) {
      ...
      for (j in 1:K) {    // j = previous (t-1)
        for (i in 1:K) {  // i in next (t)
          ...
          if(j == 1) {
            mu_gen[1] = alpha_air + A[2]*cos(2*pi()*t/n + tau_est[1]*pi());
            season_gen[1] = (cos(2*pi()*t/n + tau_est[1]*pi())+2) * sigma[2];
            accumulator[i] = logbeta[t,i] + log(A_ij[j,i]) + normal_lpdf(y[t]| mu_gen[1], season_gen[1]);
          }
          if(j == 2){
            mu_gen[2] = alpha_water + A[1]*cos(2*pi()*t/n + tau_est[2]*pi());
            season_gen[2] = (cos(2*pi()*t/n + tau_est[2]*pi())+2) * sigma[1];
