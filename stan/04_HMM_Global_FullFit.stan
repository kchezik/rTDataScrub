...
parameters {
  ...
  vector<lower=0,upper=1>[S] snow;        // Snow effect parameter
  vector[S] sp_n;                         // Seasonal end of snow effect
  ...
  { // Forward algorithm log p(z_t = j | x_{1:t})
    ...
    for (t in 1:N) {
      ...
      if(d[t] == 1){
        ...
        if(d[t]<=sp_n[site[t]]) spring = (cos(2*pi()*d[t]/sp_n[site[t]])-1) * snow[site[t]];
        else spring = 0;
        season[2]= sigma[site[t],1];
        unalpha_tk[t,2]= log(0.5)+ student_t_lpdf(y[t]|3, mu[2] + spring, season[2]);
      }
      else {
        for (j in 1:K) {    // j = current (t) or transition state column.
          for (i in 1:K) {  // i = previous (t-1) or transition state row.
              ...
              if(j == 2){
                ...
                accumulator[i]= unalpha_tk[t-1,i] + log(A_ij[i,j]) +
                                 student_t_lpdf(y[t]|3, mu[j] + spring, season[j]);
              }
          }
        ...
        for (j in 1:K) {    // j = previous (t-1)
          for (i in 1:K) {  // i in next (t)
            ...
            if(j == 2){
              ...
              if(d[t]<=sp_n[site[t]]) spring_gen = (cos(2*pi()*d[t]/sp_n[site[t]])-1) * snow[site[t]];
              else spring_gen = 0;
